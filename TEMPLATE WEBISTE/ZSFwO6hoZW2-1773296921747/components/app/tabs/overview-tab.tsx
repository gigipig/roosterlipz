...
function NextStepSection({ isUnlocked, openUnlockModal, setActiveTab }: {
  isUnlocked: boolean;
  ...
  return (
    <section className="p-6 rounded-2xl bg-card border border-border space-y-4 card-lift">
      <h3 className="font-serif text-xl text-foreground">Ready to go deeper?</h3>
      <div className="grid sm:grid-cols-2 gap-3">
        <button
          onClick={() => isUnlocked ? setActiveTab('genetics') : openUnlockModal()}
          className="flex items-center justify-between p-4 rounded-xl border border-sage/25 bg-sage/5 hover:bg-sage/10 hover:border-sage/40 transition-all text-left group"
        >
          <div>
            <p className="text-sm font-semibold text-foreground">🧬 Full Genetic Profile</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isUnlocked ? '34+ traits across 8 metabolic categories' : 'Unlock 34+ genetic traits — £9.99'}
            </p>
          </div>
          <span className="text-muted-foreground/40 group-hover:text-sage transition-colors">→</span>
        </button>

        <button
          onClick={() => isUnlocked ? setActiveTab('foods') : openUnlockModal()}
          className="flex items-center justify-between p-4 rounded-xl border border-border bg-card/60 hover:border-foreground/20 transition-all text-left group"
        >
          <div>
            <p className="text-sm font-semibold text-foreground">🌿 Foods & Recipes</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isUnlocked ? 'Browse your ancestral food tiers and scored recipes' : 'Unlock personalised food recommendations'}
            </p>
          </div>
...
export function OverviewTab() {
  const { blended, genetics, isUnlocked, openUnlockModal, setActiveTab } = useAppState();
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Item 10 — Contextual next-step CTA */}
      <NextStepSection
