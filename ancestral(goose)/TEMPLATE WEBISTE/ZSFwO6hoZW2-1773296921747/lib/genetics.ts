...
const GENE_GLOSSARY = {
  ...
  obesity_risk: {
    term: 'Obesity Risk (FTO)',
    definition: 'A common variant near the FTO gene that reduces feelings of fullness after eating. Carriers tend to eat larger portions and are drawn to calorie-dense foods, but the effect is largely reversible through high-protein meals and structured eating.',
    gene: 'FTO gene',
    inheritance: 'Additive - each copy adds modest risk'
  },
  ...
  vitamin_d_transport: {
    term: 'Vitamin D Transport (GC)',
    definition: 'The GC protein carries vitamin D through the bloodstream to target tissues. Certain variants reduce binding protein levels, so less vitamin D reaches cells even with adequate sun exposure or supplementation.',
    gene: 'GC gene',
    inheritance: 'Additive - affects binding protein levels'
  },
  salt_sensitivity: {
  ...
  vitamin_b12_absorption: {
    term: 'Vitamin B12 Absorption (FUT2)',
    definition: 'FUT2 determines secretor status, which shapes the gut environment for B12 absorption. Secretors have lower circulating B12 on average — not from absorbing less, but because gut microbiome interactions with B12-binding proteins differ.',
    gene: 'FUT2 gene',
    inheritance: 'Recessive - two copies for full non-secretor effect'
  },
  appetite_regulation: {
    term: 'Appetite Regulation (MC4R)',
    definition: 'MC4R is a brain receptor that signals fullness after meals. Variants that reduce its activity blunt the \"I am satisfied\" message, causing people to eat more at each sitting. High-protein meals activate an alternative satiety pathway that bypasses this deficit.',
    gene: 'MC4R gene',
    inheritance: 'Additive - each copy moderately reduces satiety signalling'
  },
  hdl_metabolism: {
    term: 'HDL Cholesterol (LIPC)',
    definition: 'Hepatic lipase (LIPC) breaks down HDL particles in the liver. Variants that reduce enzyme activity affect how efficiently HDL is processed and how cholesterol levels respond to dietary fat — with some genotypes behaving opposite to typical dietary advice.',
    gene: 'LIPC gene',
  ...
  fut2_b12: {
    term: 'B12 Absorption (FUT2)',
    definition: 'FUT2 determines secretor status, which shapes the gut lining environment. Secretors tend to have lower circulating B12 due to differences in gut microbiome interactions with B12-binding proteins — making regular B12-rich food or supplementation especially important.',
    gene: 'FUT2 gene',
    inheritance: 'Recessive - two non-secretor alleles for non-secretor status'
  },
  gc_vitamin_d: {
...
const NUTRIENT_GAP_CONFIG = [
  ...
  { gene: 'folate_metabolism', nutrient: 'Folate (as methylfolate)', unit: 'mcg', standardRDA: 400, modifiedRDA: '600-800', direction: 'increase', triggerMin: 40, reason: 'MTHFR variants reduce folate processing efficiency' },
  ...
  { gene: 'iron_metabolism', nutrient: 'Iron', unit: 'mg', standardRDA: 18, modifiedRDA: '8', direction: 'decrease', triggerMin: 5, reason: 'HFE variants increase iron absorption — lower intake prevents overload' },
...
const DAILY_GUIDELINE_RULES = [
  ...
  { gene: 'iron_metabolism', triggerMin: 5, period: 'all', type: 'limit', note: 'Iron overload risk — limit red meat to 2-3 times per week', foods: ['red meat', 'liver', 'organ meats'] },
